...
const fetchFormulaData = async () => {
    try {
        const response = await fetch('https://sheets.livepolls.app/api/spreadsheets/db23e7b2-bdf8-464d-83f7-75c32213a8f4/Sheet1');
...
const updateCard = (term) => {
    currentCard = formulaData.find(item => item.Front.toLowerCase() === term.toLowerCase());
    isFlipped = false;
    drawCard();
};

const updateDropdown = (searchTerm) => {
    const matchingTerms = formulaData.filter(item => 
...
// Initialize the game
(async () => {
    formulaData = await fetchFormulaData();
    updateDropdown('');
    drawCard();
})();
